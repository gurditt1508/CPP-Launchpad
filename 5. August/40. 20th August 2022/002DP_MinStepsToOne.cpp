...
int minSteps(int n) {
	...
	// 1. ask your friend to count the min. steps required to reduce n-1 to 1
	int X = minSteps(n-1);

	// 2. ask your friend to count the min. steps required to reduce n/2 to 1
	int Y = INT_MAX; //if number is never divisible by 2, we need to make sure Y is never the answer
	if(n%2 == 0) {
		Y = minSteps(n/2);
	}

	// 3. ask your friend to count the min. steps required to reduce n/3 to 1
	int Z = INT_MAX;
	if(n%3 == 0) {
		Z = minSteps(n/3);
	}
...
int minStepsBottomUp(int n) {
	...
	vector<int> dp(n+1); // 0th idx will be unused

	dp[1] = 0;
	for(int i=2; i<=n; i++) {
		// dp[i] -> min steps required to reduce i to 1
		int X = dp[i-1];
		int Y = INT_MAX;
		if(i%2 == 0) {
			Y = dp[i/2];
